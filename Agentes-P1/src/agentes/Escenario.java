...
public class Escenario extends JFrame
{
    ...
    public Escenario()
    {
        ...
        try
        {    
            AudioInputStream inputStream = AudioSystem.getAudioInputStream(new File("audio/audio.wav"));
    ...
    public void insertaObjeto(MouseEvent e)
    {
        ...
        int fila = (casilla.getY() - 10) / 50; // Obtiene la fila de la casilla
        int columna = (casilla.getX() - 10) / 50; // Obtiene la columna de la casilla
        if(actualIcon!=null) casilla.setIcon(actualIcon); 
        //agregando valor a los obstaculos, muestras y la nave nodriza
        if(actualIcon == obstacleIcon){
            matrix[fila][columna] = 5;
        }else if(actualIcon == sampleIcon){
            matrix[fila][columna] = 6;
        }else if(actualIcon == motherIcon){
            matrix[fila][columna] = 7;
        }
    }
